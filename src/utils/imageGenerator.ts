...
export async function generateRegistrationImage(data: RegistrationFormData): Promise<void> {
  return new Promise((resolve) => {
    ...
    const docSections: Array<{ type: "title" | "subtitle" | "field" | "bullet" | "paragraph" | "header" | "bold"; text: string }> = [
      ...
      { type: "bold", text: `Cidade: ${(data.cidadeEquipe || data.cidadeEstadoEquipe || "").toUpperCase()}` },
      { type: "bold", text: `Estado: ${(data.estadoEquipe || "").toUpperCase()}` },
      ...
      { type: "bullet", text: `Cidade: ${data.capitao.cidade || data.capitao.cidadeEstado || "-"}` },
      { type: "bullet", text: `Estado: ${data.capitao.estado || "-"}` },
      ...
      { type: "bullet", text: `Cidade: ${data.segundoPescador.cidade || data.segundoPescador.cidadeEstado || "-"}` },
      { type: "bullet", text: `Estado: ${data.segundoPescador.estado || "-"}` },
      ...
      { type: "bullet", text: `Cidade: ${p3 ? (p3.cidade || p3.cidadeEstado || "-") : "-"}` },
      { type: "bullet", text: `Estado: ${p3 ? (p3.estado || "-") : "-"}` },
